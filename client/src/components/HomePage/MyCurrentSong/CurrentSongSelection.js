  ...
  render() {
    ...
    return (
       <div id="song-selection" style={{display: 'flex', flexDirection: 'column', backgroundColor: '#eff0f2', width: '100%', height:'50%', alignItems: 'center'}}>
        ...
        <img style={{padding: '2em 0 0 0'}} src={this.props.selectedSong.trackImage300 || this.props.currentAlbumArtwork}/>
        <div style={{fontSize: '40px', padding: '.5em 0 0 0'}}>{this.props.selectedSong.trackName || this.props.currentMySong.trackName}</div>
        <div style={{fontSize: '20px', padding: '5px .5px'}}>{this.props.selectedSong.trackArtist || this.props.currentMySong.trackArtist}</div>
       </div>
    )
  }
}
